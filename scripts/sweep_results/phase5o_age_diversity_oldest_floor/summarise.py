#!/usr/bin/env python3
"""Aggregate the fifo vs age-diversity-oldest-floor A/B sweep into a summary."""
...
def _delta_by_task(fifo: dict, cand: dict) -> dict[str, float]:
    fifo_mean = _per_task_mean(fifo["results"])
    cand_mean = _per_task_mean(cand["results"])
    return {
        task: cand_mean[task] - fifo_mean[task]
        for task in sorted(set(fifo_mean) & set(cand_mean), key=int)
    }
...
def main() -> int:
    ...
    summary = {
        "experiment": "phase5o_age_diversity_oldest_floor_ab",
        "created_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "fifo": fifo,
        "age_diversity_oldest_floor": cand,
        "delta_mean_acc": (
            (cand["avg_accuracy_mean"] - fifo["avg_accuracy_mean"])
            if fifo["avg_accuracy_mean"] is not None
            and cand["avg_accuracy_mean"] is not None
            else None
        ),
        "delta_mean_forgetting": (
            (cand["avg_forgetting_mean"] - fifo["avg_forgetting_mean"])
            if fifo["avg_forgetting_mean"] is not None
            and cand["avg_forgetting_mean"] is not None
            else None
        ),
        "delta_final_task_accuracy_mean": _delta_by_task(fifo, cand),

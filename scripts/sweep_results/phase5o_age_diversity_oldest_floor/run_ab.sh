...
OLDEST_FLOOR="${POOL_OLDEST_FLOOR:-2}"
mkdir -p "$RESULT_DIR"
for policy in fifo age-diversity-oldest-floor; do
  for seed in 1 2 3 4 5; do
    ...
    echo "[$(date -Is)] policy=${policy} seed=${seed} floor=${OLDEST_FLOOR} start" \
      | tee -a "$RESULT_DIR/progress.log"
    python3 scripts/run_split_mnist.py mnist \
      ...
      --pool-evict-policy "$policy" --pool-oldest-floor "$OLDEST_FLOOR" \
...
python3 scripts/sweep_results/phase5o_age_diversity_oldest_floor/summarise.py

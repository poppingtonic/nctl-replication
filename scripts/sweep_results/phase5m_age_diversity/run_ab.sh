#!/usr/bin/env bash
# 30.13 A/B sweep: fifo (regression parity) vs age-diversity task-free retention.
#
...
# phase5k_task_floor's 90.94% +/- 0.31 control mean before the age-diversity
# delta is interpreted.
set -euo pipefail
cd "$(dirname "$0")/../../../.."
RESULT_DIR="scripts/sweep_results/phase5m_age_diversity"
mkdir -p "$RESULT_DIR"
for policy in fifo age-diversity; do
...
python3 scripts/sweep_results/phase5m_age_diversity/summarise.py

...
SCRIPT = (
    ...
    / "sweep_results"
    / "phase5m_age_diversity"
    / "summarise.py"
)


def _load_module():
    spec = importlib.util.spec_from_file_location("phase5m_summarise", SCRIPT)
...
def test_phase5m_summary_reports_policy_and_task_deltas(tmp_path: Path) -> None:
    mod = _load_module()
    mod.ROOT = tmp_path
    _write_result(tmp_path, "fifo", 1, 90.0, 80.0)
    _write_result(tmp_path, "fifo", 2, 92.0, 82.0)
    _write_result(tmp_path, "age-diversity", 1, 95.0, 88.0)
    _write_result(tmp_path, "age-diversity", 2, 97.0, 90.0)

    assert mod.main() == 0

    summary = json.loads((tmp_path / "summary_ab.json").read_text())
    assert summary["experiment"] == "phase5m_age_diversity_ab"
    assert summary["delta_mean_acc"] == 5.0
    assert summary["delta_final_task_accuracy_mean"]["1"] == 8.0
    assert summary["fifo"]["n"] == 2
    assert summary["age_diversity"]["n"] == 2
